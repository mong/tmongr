...
test_that("filterBehandlingsniva is correct", {
  ...
  for (behniva in list("Døgnopphold", "Dagbehandling", "Poliklinikk","Avtalespesialist",
    ...
    k <- k + 1
    expect_equal_to_reference(filterBehandlingsniva(testdata, behniva),
                              paste0("data/unit_behniva_", k, ".rds"))
    expect_equal_to_reference(filterBehandlingsniva(testdata2, behniva),
...
test_that("filterICD10 is correct", {
  ...
  k <- 0
  for (icd10 in list(
    "Visse infeksjonssykd og parasittsykd",
    "Svulster",
